      ...
      <div>
        {character.miscData?.extras?.length ? (
          <div>
            ...
            <PillFilter
              pills={Object.keys(pillTypes).map(
                key => `${ResultType[Number(key)]}`,
              )}
            >
              {Object.entries(pillTypes).map(([key, items]) => (
                <ContentBlock key={key} name={`${ResultType[Number(key)]}`}>
                  {items.map(item => (
                    <div
                      key={item.name}
                    ...
                    >
                      <div className="py-2 px-2 flex justify-between w-full">
                        <div className="flex items-center">
                          ...
                          <div className={`rating-${item.rating}`}>{`${
                            item.name
                          } ${item.baseName ? `(${item.baseName})` : ''}`}</div>
